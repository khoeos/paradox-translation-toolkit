/// <reference lib="dom" />
// This file speaks HTTP, so it needs `AbortSignal` and `URL` as VALUES, not just as types.
// Referenced here rather than set in a tsconfig, so the requirement travels with the source: this
// package points `main` at `./src`, so every consumer typechecks this very file inside its own
// program and would otherwise have to know to allow node globals (that is exactly why `@ptt/report`
// briefly needed `"types": ["node"]` for something none of its own files uses).
// `lib="dom"` and not `types="node"`: it supplies the two web-standard globals without re-admitting
// `node:fs`, `process` and `Buffer`, which is what the old `/// <reference types="node" />` did.
