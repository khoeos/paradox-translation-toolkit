...
describe('buildTargetContent', () => {
  ...
  it('keeps a key holding the source language in its name (S-1)', async () => {
    // The original retagged with content.replaceAll('l_english', 'l_russian') over the whole
    // file, so this key was renamed, stopped matching job.keys and vanished with no trace.
    const fs = new MemoryFs({
  ...
  it('writes only the keys the job asks for', async () => {
    // Anything else is covered by somebody else, and writing it would shadow their work.
    const fs = new MemoryFs({
      'mod/localisation/a_l_english.yml': localeFile('english', [
        ['MINE', 'a'],
        ['THEIRS', 'b']
  ...
  it('keeps what an earlier run translated over a fresh translation', async () => {
    // It is already in the file being rewritten, and was never sent to a translator twice.
    const fs = new MemoryFs({
      'mod/localisation/a_l_english.yml': localeFile('english', [['K', 'Colony Ship']])
    })
    const content = await buildTargetContent(
  ...
  it('drops the comments of the source file', async () => {
    // The generated file is ours; a comment about the English text is noise in Russian.
    const fs = new MemoryFs({
      'mod/localisation/a_l_english.yml': `﻿l_english:\n # a note\n K:0 "text"\n`
    })
    const content = await buildTargetContent(
  ...
  it('produces a file the parser reads back', async () => {
    ...
    const { parse } = await import('@ptt/parser-core')
