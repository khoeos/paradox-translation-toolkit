...
/**
 * The FS contract now lives in `@ptt/shared`, and is re-exported here so the ~30 sites that say
 * `import type { FsLike } from './types.js'` are unchanged.
 *
 * It was declared in this file, which meant `@ptt/translate`, `@ptt/report` and `@ptt/fs-node`
 * each had to depend on the converter to name a filesystem. `@ptt/fs-node` pointing at the cores
 * while every core pointed at `@ptt/shared` is what made the graph impossible to fold.
 */
 ...
 * needs (`domain`) must not ripple into every converter fixture.
 */
 ...
 *
 * A port rather than the class: `converter` must not depend on the translation
