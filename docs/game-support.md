...
| `languageFileToken`   | The token in `<key>_l_<token>.yml`. Stellaris uses `braz_por`, CK3 uses `simp_chinese`, etc., never assume. This is the **built-in** mapping only, one token per shipped language, and it is also the **only** set of tokens a user can pick a custom target's file token from: the token `Select` in the custom-target dialog, the CLI's `:<token>` grammar and the tRPC boundary all reject a token that isn't one of this map's values. A user picking a custom target (a language written under another token, e.g. Catalan saved as `l_english`) never mutates this map or lands in a game definition, it stays static build-time data; `TranslationTarget` carries the token per run instead. A game definition that lags behind a game update (a token the game now ships but this map doesn't list yet) is a user-visible gap: that token is simply not offered until the definition is updated. |
