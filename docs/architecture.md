...
- **Disk is keyed by the game's own tokens, meaning is a free label.** A `TranslationTarget` is `{ language, fileToken }`. `language` is free text: normalized to a `LanguageCode` when recognized (`normalizeTargetLanguage`), kept verbatim otherwise, and a run holds at most one target per _normalized_ language (`findTargetListProblem` rejects a second). `fileToken` must be one of the values of `gameDef.languageFileToken` (R1): the token grammar (`isFileToken`, `normalizeFileToken`) is kept as defence in depth against hand-edited or persisted junk, but the game's declared token set is the rule a user-facing surface enforces. The source language side stays closed (`sourceLanguage: LanguageCode` everywhere): only the target side is free text. `gameDef.languageFileToken` is consulted for target resolution in `resolveTargets` (`packages/converter/src/target.ts`), in `toGameSummary` (`packages/games/src/index.ts`), and by the glossary. Everywhere else stays language-keyed with `string`, not `LanguageCode` (`ModKeys.byLanguage`, `Coverage.byLanguage`, `ModResult.created`) except `GeneratedMod`'s inner map, which is token-keyed because our own output under a non-owned token can only be found again by its token. Normalization happens once, at the boundaries listed in `TranslationTarget`'s own module doc (the custom-target dialog, both tRPC procedures' refine and mutation body, the CLI's `parseTargets`, and defensively inside `resolveTargets` and the run-level list in `runConvert`/`scanMods`), which is what lets `usesOwnToken` and `shadowedLanguageOf` stay pure, unrecognized-free comparisons instead of calling the recognizer themselves.
